...
def get_centerlines_rgi(dem_path, outline):
    ...
    maxorder = np.rint(cfg.PARAMS['localmax_window']/dx)
    ...
    radius = cfg.PARAMS['q1'] * area + cfg.PARAMS['q2']
    radius = utils.clip_scalar(radius, 0, cfg.PARAMS['rmax'])
    ...
    olines, oheads = _filter_lines1(lines, heads_pix, cfg.PARAMS['kbuffer'], radius)
    
    
    # Filter the lines which are going up instead of down
    min_slope = np.deg2rad(cfg.PARAMS['min_slope_flowline_filter'])
    
    if cfg.PARAMS['filter_min_slope']: # this kills most of the branches! --> too many killed!
        topo = z
        olines, oheads = _filter_lines_slope1(olines, oheads, topo,
                                             gdir, min_slope)
    # tmp: convert to cls
...
cfg.add_to_basenames('centerlines_rgi', 'centerlines_rgi.shp', 'Centerlines computed_differently')#docstr='Centerlines computed_differently')
...
def compute_centerlines_rgi(gdir):
    ...
    # call your tool
    clsgpd = get_centerlines_rgi(dem_path, outline)
    
    # now you have the centerlines cls
    clsgpd = clsgpd

    # Write them
    gdir.write_shapefile(clsgpd, 'centerlines_rgi')
